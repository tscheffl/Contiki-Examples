...
RESOURCE(json_test, METHOD_GET, "json_test");

/* For each resource defined, there corresponds an handler method which should be defined too.
 * Name of the handler method should be [resource name]_handler
 * */
void
json_test_handler(REQUEST* request, RESPONSE* response)
{
  call_nr++;
  sprintf(temp,"{\n\"test\" : \"%d\"\n}",call_nr%5);

  rest_set_header_content_type(response, APPLICATION_JSON);
  rest_set_header_access_control_allow_origin(response, "*");
  rest_set_response_payload(response, (uint8_t*)temp, strlen(temp));
}
...
PROCESS_THREAD(rest_server_example, ev, data)
{
  ...
  rest_activate_resource(&resource_json_test);
