...
int
rest_set_header_access_control_allow_origin(RESPONSE* response, const char* access_string)
{
#ifdef WITH_COAP
  //return coap_set_header_content_type(response, content_type);
#else
  return http_set_res_header(response, HTTP_HEADER_NAME_ACCESS_CONTROL_ALLOW_ORIGIN, access_string, 1);
#endif /*WITH_COAP*/
}
